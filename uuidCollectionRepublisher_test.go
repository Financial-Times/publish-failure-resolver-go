...
func TestRepublishOk_Ok(t *testing.T) {
	...
	mockedNotifierClient.On("Notify", []byte("native"), "cms-notifier", "methode-web-pub", "application/json", "f3b3b579-732b-4323-affa-a316aacad213", "tid_123").Return(nil)
...
func TestRepublishErrNotifier_Err(t *testing.T) {
	...
	mockedNativeStoreClient.On("GetNative", "methode", "f3b3b579-732b-4323-affa-a316aacad213", "tid_123").Return([]byte("native"), true, nil)
	mockedNotifierClient := new(mockNotifierClient)
	mockedNotifierClient.On("Notify", []byte("native"), "cms-notifier", "methode-web-pub", "application/json", "f3b3b579-732b-4323-affa-a316aacad213", "tid_123").Return(fmt.Errorf("error on notifier maybe 404"))
	republisher := newNotifyingUCRepublisher(mockedNotifierClient, mockedNativeStoreClient, 500*time.Millisecond)
	collection := targetSystem{
		name:           "methode",
		originSystemID: "methode-web-pub",
...
func (m *mockNotifierClient) Notify(nativeContent []byte, notifierApp, originSystemID, contentType, uuid, tid string) error {
	args := m.Called(nativeContent, notifierApp, originSystemID, contentType, uuid, tid)
	return args.Error(0)
}
